...
private:
  ...
  {
    if (payload.empty()) {
      RCLCPP_WARN(this->get_logger(), "Skip publishing jetson/alarm: empty payload");
      return;
    }

    if (client_ == nullptr || !client_->is_connected()) {
      RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 3000, "Cannot publish jetson/alarm, MQTT not connected");
      return;
    }

    try {
      auto msg = mqtt::make_message(alarm_topic_, payload, 1, false);
      client_->publish(msg)->wait();
    }
    catch (const mqtt::exception& e) {
      RCLCPP_ERROR(this->get_logger(), "Publish jetson/alarm failed: %s", e.what());
    }
  }
  ...
  std::string alarm_topic_;        ///< 告警发布主题
  ...
  rclcpp::Subscription<std_msgs::msg::String>::SharedPtr alarm_event_sub_; ///< 告警事件订阅器
